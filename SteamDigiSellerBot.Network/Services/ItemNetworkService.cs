...
namespace SteamDigiSellerBot.Network.Services
{
    ...
    public class ItemNetworkService : IItemNetworkService
    {
        ...
        public async Task GroupedItemsByAppIdAndSetPrices(List<Item> items, string aspNetUserId, bool reUpdate = false, Dictionary<int, decimal> prices = null)
        {
            ...
            var skipNum = 0;
            var chunkSize = (int) Math.Ceiling(CountRecomendationChankSize(proxyCount, ProxyPull.MAX_REQUESTS, currenicesCount) / 2d);
            ...
            foreach (Item item in dbItems)
            {
                ...
                else
                {
                    ...
                    if (( item.CurrentDigiSellerPrice != digiPriceWithAllSalesInRub || reUpdate || (prices != null && ids.Any(id => prices.ContainsKey(id) && prices[id] != digiPriceWithAllSalesInRub))) && currentSteamPrice > 0)
                    {
                        ...
                        {
                            _logger.LogWarning($"SetPrices: Установка стоимости на товар {appId} - {item.Id} в {digiPriceWithAllSalesInRub} ({(digiPriceWithAllSalesInRub / item.CurrentDigiSellerPrice * 100):0.0}%)");
                            item.CurrentDigiSellerPriceNeedAttention = true;
                        }
                        else
                        {
