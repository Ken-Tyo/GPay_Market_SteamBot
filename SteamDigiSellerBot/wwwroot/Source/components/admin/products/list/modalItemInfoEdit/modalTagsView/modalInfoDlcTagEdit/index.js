...
import { apiFetchTagInfoDlcReplacementValues, apiTagInfoDlcReplacementValues, apiFetchMarketPlaces, apiFetchLanguages } from '../../../../../../../containers/admin/state'

const ModalInfoDlcTagEdit = ({ isOpen, onClose }) => {
  if (!isOpen)
    return;
  ...
  useEffect(() => {
    apiFetchLanguages().then((langData) => {
      ...
      apiFetchTagInfoDlcReplacementValues().then((data) => {
        ...
        for (var item of data) {
          ...
          for (var tagInfoDlcReplacementValue of item.replacementValues) {
            if (!tagInfoDlcReplacementValue.value) {
              continue;
            }

            valuesMap.set(`${tagInfoDlcReplacementValue.languageCode}`, tagInfoDlcReplacementValue.value)
          }
        }
  ...
  const onSave = async () => {
    ...
    await apiTagInfoDlcReplacementValues({ values: data });
    onClose();
  }
  ...
  return (
    <ModalBase
      isOpen={isOpen}
      className={css.tagInfoDlcEditModalContent}
      width={'890px'}
      height={'auto'}
    >
      <div className={css.title}><h1>Теги - %infoDLC%</h1></div>
      ...
      <div className={css.tagInfoDlcEditButtons}>
...
export default ModalInfoDlcTagEdit;